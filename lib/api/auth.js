...
const API_BASE_URL = process.env.NEXT_PUBLIC_AUTH_API_URL || 'https://fbauth.rakibulhoque.com/api/v1'

// Token management functions (sync with main api.js)
...
// Sync tokens between localStorage and cookies for compatibility
const syncTokensWithCookies = () => {
  if (typeof window === 'undefined') return
  ...
  try {
    ...
    import('js-cookie').then(({ default: Cookies }) => {
      ...
      if (token) {
        Cookies.set('fabrimentory_access_token', token, { expires: 7 })
      }
      if (refreshToken) {
        Cookies.set('fabrimentory_refresh_token', refreshToken, { expires: 30 })
      }
    })
  } catch (error) {
    console.warn('Could not sync tokens with cookies:', error)
  }
}
...
class AuthApiService {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 && !isRetry) {
          ...
          try {
            ...
            // Retry the original request with new token using the new token
            return this.request(endpoint, options, true);
          } catch (refreshError) {
  ...
  setTokens(tokens) {
    ...
    // Sync with cookies for main API service compatibility
  ...
  clearTokens() {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('fabrimentory_token_expires')
    }
  }
